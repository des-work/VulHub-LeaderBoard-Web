...
const RippleGrid = ({
  enableRainbow = false,
  gridColor = '#00ff00', // Matrix green by default
  ...
  useEffect(() => {
    ...
    loadOGL().then(({ Renderer, Program, Triangle, Mesh }) => {
      if (!containerRef.current) return;
    ...
    <div ref={containerRef} className="ripple-grid-container">
      {!isLoaded && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <div className="text-green-400 font-mono text-sm">Loading RippleGrid...</div>
        </div>
      )}
    </div>
  );
